...
import org.ovi.databinding.FragmentRegEducationBinding
...
class RegEducationFragment : BaseFragment<FragmentRegEducationBinding>() {
    ...
    override fun getViewBinding() = FragmentRegEducationBinding.inflate(layoutInflater)


    override fun setupView() {
        with(binding.include) {
            tvTitle.text = getString(R.string.what_is_your_highest_educatiion)
            rvRecycle.apply {
                verticalView(context)
//                adapter= RegListAdapter(::callback, DataUtil.getEducation())
                adapter = RegListAdapter(
                    ::callback,
                    data.choices?.map { SelectionModel(it?.choice.toString(), false) })
