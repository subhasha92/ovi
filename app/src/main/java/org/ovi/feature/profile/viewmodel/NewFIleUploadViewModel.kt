...
import org.ovi.feature.profile.domain.INewFileUploadRepo
...
class NewFIleUploadViewModel(private val iNewFileUploadRepo: INewFileUploadRepo):BaseViewModel() {
    ...
    fun uploadMultipleFiles(uriList : ArrayList<Uri>, processFile : Boolean = true){
        execute {
            ...
            uriList.forEach {
                kotlin.runCatching {
                    if(processFile)
                        iNewFileUploadRepo.uploadImage(processFile(it))
                    else iNewFileUploadRepo.uploadImage(it.path)
                }.onSuccess {
                    if(it.status == 1)
    ...
    fun uploadFile(uri : Uri, processFile : Boolean = true){
        execute {
            ...
            kotlin.runCatching {
                if(processFile)
                    iNewFileUploadRepo.uploadImage(processFile(uri))
                else iNewFileUploadRepo.uploadImage(uri.path)
            }.onSuccess {
                if(it.status == 1)
